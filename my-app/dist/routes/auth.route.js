...
const toStorageSafeProfilePicture = (value) => {
...
const buildStorageSafeUser = (user) => {
...
authRoute.post('/register', async (c) => {
    ...
    try {
        ...
        await pool.query('INSERT INTO register (fullname, email, sex, department, contact_number, password) VALUES (?, ?, ?, ?, ?, ?)', [fullname, email, sex, department, contact_number, hashedPassword]);
...
authRoute.get('/profile/:id', async (c) => {
    ...
    try {
        const [rows] = await pool.query('SELECT id, fullname, sex, department, contact_number, email, profile_picture, profile_picture AS profilePicture FROM register WHERE id = ?', [id]);
        if (rows.length === 0) {
...
const updateProfileHandler = async (c) => {
    const id = c.req.param('id');
    const body = await c.req.json();
    const { fullname, email, sex, department, contact_number, profile_picture, profilePicture } = body;
    try {
        const [existingRows] = await pool.query('SELECT id, fullname, sex, department, contact_number, email, profile_picture FROM register WHERE id = ?', [id]);
        ...
        await pool.query('UPDATE register SET fullname=?, email=?, sex=?, department=?, contact_number=?, profile_picture=? WHERE id=?', [nextFullname, nextEmail, nextSex, nextDepartment, nextContactNumber, nextProfilePicture, id]);
