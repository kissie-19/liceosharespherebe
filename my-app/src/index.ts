...
async function ensureAdminUser() {
  try {
    const [rows] = await pool.query<any[]>('SELECT id FROM register WHERE email = ?', [ADMIN_EMAIL])
    const hashedPassword = await bcrypt.hash(ADMIN_PASSWORD, 10)

    if (rows.length === 0) {
      await pool.query(
        'INSERT INTO register (fullname, email, sex, department, contact_number, password) VALUES (?, ?, ?, ?, ?, ?)',
        ['Admin', ADMIN_EMAIL, 'Male', 'College of Information Technology', '09123456789', hashedPassword]
      )
